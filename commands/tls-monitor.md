...
Call `tlsradar.monitor_list`. Show domain, expiry days, last-scan timestamp. Sort by days-until-expiry ascending so the most urgent surfaces first.

If the user is at the plan's monitor cap (used == limit from `monitor_list` meta), mention casually: "You're at your `<plan>` plan's `<limit>` monitor cap. `/tls-upgrade` to lift it."

## `add <domain> [more domains...]`

Decide between single-add and bulk-add based on argument count:

- **One domain:** call `tlsradar.monitor_add` with `domain`
- **Two or more:** call `tlsradar.monitor_add_bulk` with `domains: [d1, d2, ...]`. The bulk variant returns per-domain status, so you can render "added 8/10, 2 hit the limit."
...
Call `tlsradar.monitor_remove` with `domain`. Confirm with the user before calling if removing would drop their monitor count to 0 - getting back to 1 just means re-adding, but a user might not realize they're about to lose their only monitor.

## Things this command should NOT do

- Don't loop: if `monitor_add_bulk` hits the limit halfway through, surface the result once; don't keep retrying.
...
- Don't auto-suggest team-scoping operations - if the user wants to invite a teammate, that's the `tlsradar.team_invite` tool (no slash command), reached by natural language.
